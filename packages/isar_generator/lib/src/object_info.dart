...
class ObjectInfo {
  ...
  int get id => xxh3(Uint8List.fromList(utf8.encode(isarName)));
...
class ObjectIndex {
  ...
  late final id = xxh3(Uint8List.fromList(utf8.encode(name)));
}
...
class ObjectLink {
  ...
  int id(String objectIsarName) {
    ...
    final colId = xxh3(
      Uint8List.fromList(utf8.encode(col)),
      seed: isBacklink ? 1 : 0,
    );

    final name = targetLinkIsarName ?? isarName;
    return xxh3(Uint8List.fromList(utf8.encode(name)), seed: colId);
  }
}
