...
/// Custom painter for the circular duration dial
class _DurationDialPainter extends CustomPainter {
  _DurationDialPainter({required this.duration, required this.isDark});
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // Background circle
    final bgPaint = Paint()
      ..color = (isDark ? Colors.white : AppTheme.electricViolet).withOpacity(0.1)
    ...
    // Calculate sweep angle (0-1440 minutes = 0-360 degrees)
    final sweepAngle = (duration / 1440) * 2 * math.pi;
    canvas.drawArc(
    ...
    for (int i = 0; i < 24; i++) {
      ...
      final markerPaint = Paint()
        ..color = (isDark ? Colors.white : AppTheme.electricViolet).withOpacity(
          i % 6 == 0 ? 0.6 : 0.3,
        )
    ...
    // Draw handle
    final handleAngle = (duration / 1440) * 2 * math.pi - math.pi / 2;
    ...
    final handleBorderPaint = Paint()
      ..shader = LinearGradient(
        colors: [AppTheme.electricViolet, Colors.pink],
      ).createShader(Rect.fromCircle(center: handlePos, radius: 14))
      ..style = PaintingStyle.stroke
      ..strokeWidth = 3;
  ...
  @override
  bool shouldRepaint(_DurationDialPainter oldDelegate) {
    return oldDelegate.duration != duration || oldDelegate.isDark != isDark;
  }
}

/// Custom painter for the clock-style target time dial
class _ClockDialPainter extends CustomPainter {
  _ClockDialPainter({required this.targetTime, required this.isDark});
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < 24; i++) {
      ...
      // Draw hour labels (every 6 hours: 0, 6, 12, 18)
      if (i % 6 == 0) {
        ...
        final textPainter = TextPainter(
          text: TextSpan(
            text: '$i',
            style: TextStyle(
              color: (isDark ? Colors.white : AppTheme.electricViolet).withOpacity(0.6),
              fontSize: 12,
              fontWeight: FontWeight.w600,
            ),
          ),
    ...
    // Draw arc from current time to target time
    final now = TimeOfDay.now();
    final currentAngle = ((now.hour + now.minute / 60) / 24) * 2 * math.pi - math.pi / 2;
    final targetAngle = ((targetTime.hour + targetTime.minute / 60) / 24) * 2 * math.pi - math.pi / 2;

    var sweepAngle = targetAngle - currentAngle;
    ...
    final currentPaint = Paint()
      ..color = (isDark ? Colors.white : AppTheme.electricViolet).withOpacity(0.5)
      ..style = PaintingStyle.fill;
    canvas.drawCircle(currentPos, 6, currentPaint);
  }

  @override
  bool shouldRepaint(_ClockDialPainter oldDelegate) {
    return oldDelegate.targetTime != targetTime || oldDelegate.isDark != isDark;
  }
}
